...
switch(op)
{
case "sum": res = nums.reduce(function(a,b){ return a+b; },0); break;
case "mean": res = nums.reduce(function(a,b){ return a+b; },0)/nums.length; break;
case "median": let mid = Math.floor(nums.length/2); if(nums.length%2!=0) res = nums[mid]; else res = (nums[mid-1]+nums[mid])/2; break;
case "mode": let freq = {}; nums.forEach(function(n){ freq[n]=(freq[n]||0)+1; }); let maxf = Math.max.apply(null,Object.values(freq)); res = Object.keys(freq).filter(function(k){ return freq[k]==maxf; }).join(','); break;
case "variance": let m = nums.reduce(function(a,b){ return a+b; },0)/nums.length; res = nums.reduce(function(a,b){ return a+Math.pow(b-m,2); },0)/nums.length; break;
case "std": let m2 = nums.reduce(function(a,b){ return a+b; },0)/nums.length; res = Math.sqrt(nums.reduce(function(a,b){ return a+Math.pow(b-m2,2); },0)/nums.length); break;
}
